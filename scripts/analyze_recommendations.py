...
def analyze_recommendations():
    ...
    # Analyze the data
    if (
        "Monthly savings" not in df.columns
        or "Type" not in df.columns
        or "Impact" not in df.columns
    ):
        print(
            "⚠️ Recommendations data is missing required columns ('Monthly savings', 'Type', 'Impact')."
        )
        return

    # Group by recommendation type and impact
    savings_summary = (
        df.groupby(["Type", "Impact"])["Monthly savings"].sum().unstack(fill_value=0)
    )
