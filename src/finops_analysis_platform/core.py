...
class MachineTypeDiscountMapping:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        if config_path is None:
            config_path = (
                Path(__file__).parent / "config" / "machine_discounts.yaml"
            )
    ...
    def _extract_machine_base(self, machine_type: str) -> str:
        ...
        if 'ssd' in machine_type:
            return 'local-ssd'
        logger.debug(
            "Could not determine base type for '%s', defaulting to 'n2'.",
            machine_type
        )
        return 'n2'
...
class CUDAnalyzer:
    ...
    def generate_comprehensive_analysis(self) -> Dict[str, Any]:
        ...
        savings_by_machine = self._calculate_savings_by_machine(
        ...
        ai_portfolio = self.generate_cud_portfolio_optimization(
            savings_by_machine
        )
    ...
    def _analyze_machine_distribution(self) -> Dict[str, float]:
        ...
        for machine_type, cost in grouped.items():
            ...
            base_type = self.discount_mapping._extract_machine_base(
                str(machine_type)
            )
    ...
    def _calculate_savings_by_machine(
        ...
        for machine_type, monthly_spend in distribution.items():
            ...
            discounts = {
                ...
                '1yr_flex': self.discount_mapping.get_discount(
                    machine_type, '1yr_flex'
                ) or 0,
                '3yr_flex': self.discount_mapping.get_discount(
                    machine_type, '3yr_flex'
                ) or 0,
            }
            savings[machine_type] = {
                ...
                'savings_options': {
                    key: {
                        'discount': value,
                        'monthly_savings': stable_workload * value
                    }
    ...
    def _generate_portfolio_recommendation(
        ...
        total_spend = sum(s['monthly_spend']
                          for s in savings_by_machine.values())

        coverage = (total_savings / total_spend * 100) if total_spend > 0 else 0
        return {
            'layers': sorted(
                layers, key=lambda x: x['monthly_savings'], reverse=True
            ),
