...
def create_dashboard(analysis: Dict[str, Any], config_manager: ConfigManager):
    ...
    theme = config_manager.get('reporting.theme_colors', {
        'primary': '#3B82F6', 'secondary': '#1E3A8A', 'accent': '#F59E0B',
        'success': '#10B981', 'danger': '#EF4444'
    })

    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=('Monthly Savings by Strategy', 'Machine Type Spend Distribution',
                        'Risk Assessment', 'Top Savings by Machine Type'),
        specs=[[{'type': 'bar'}, {'type': 'pie'}], [{'type': 'bar'}, {'type': 'bar'}]]
    )

    # 1. Savings by Strategy
    savings_summary = analysis['total_savings_summary']
    strategies = ['1-Yr Resource', '3-Yr Resource', '1-Yr Flex', '3-Yr Flex', 'Optimal Mix']
    savings = [
        savings_summary.get('1_year_resource', 0), savings_summary.get('3_year_resource', 0),
        savings_summary.get('1_year_flex', 0), savings_summary.get('3_year_flex', 0),
        savings_summary.get('optimal_mix', 0)
    ]
    fig.add_trace(go.Bar(x=strategies, y=savings, text=[f'${s:,.0f}' for s in savings],
                         textposition='auto', marker_color=[theme['primary'], theme['secondary'],
                                                            theme['success'], theme['accent'], theme['danger']]),
                  row=1, col=1)

    # 2. Machine Spend Distribution
    top_machines = sorted(analysis['machine_spend_distribution'].items(), key=lambda x: x[1], reverse=True)[:8]
    fig.add_trace(go.Pie(labels=[m[0].upper() for m in top_machines],
                         values=[m[1] for m in top_machines], hole=0.3),
                  row=1, col=2)

    # 3. Risk Distribution
    risk_dist = analysis['risk_assessment']['risk_distribution']
    fig.add_trace(go.Bar(x=['Low', 'Medium', 'High'], y=[risk_dist.get('low',0), risk_dist.get('medium',0), risk_dist.get('high',0)],
    ...
    fig.add_trace(go.Bar(x=[l['machine_type'].upper() for l in portfolio_layers],
                         y=[l['monthly_savings'] for l in portfolio_layers],
                         text=[f"${s:,.0f}" for s in [l['monthly_savings'] for l in portfolio_layers]],
                         textposition='auto', marker_color=theme['primary']),
                  row=2, col=2)

    fig.update_layout(title_text="Cloud FinOps CUD Analysis Dashboard", showlegend=False, height=800)
    return fig


class PDFReportGenerator:
    """Generates professional, customizable PDF reports for executive presentation."""

    def __init__(self, config_manager: ConfigManager):
        self.config_manager = config_manager
        self.styles = getSampleStyleSheet()
        self.theme = self.config_manager.get('reporting.theme_colors', {
    ...
    def _setup_custom_styles(self):
        ...
        self.styles.add(ParagraphStyle(name='CustomTitle', fontSize=24, textColor=HexColor(self.theme['primary']),
                                      spaceAfter=20, alignment=TA_CENTER, fontName='Helvetica-Bold'))
        self.styles.add(ParagraphStyle(name='SectionHeader', fontSize=16, textColor=HexColor(self.theme['primary']),
                                      spaceAfter=12, spaceBefore=12, fontName='Helvetica-Bold'))
        self.styles.add(ParagraphStyle(name='NormalLeft', parent=self.styles['Normal'], alignment=TA_LEFT))

    def generate_report(self, analysis: Dict[str, Any], filename: Optional[str] = None) -> str:
        """
        Generates a comprehensive, multi-page PDF report.

        Args:
            analysis: The analysis results dictionary.
            filename: The desired filename for the report. If None, a default is used.
    ...
    def _build_executive_summary(self, story: List[Any], analysis: Dict[str, Any]):
        ...
        table.setStyle(TableStyle([
            ...
            ('BACKGROUND', (0, 1), (-1, -1), HexColor(self.theme['background'])),
            ('GRID', (0, 0), (-1, -1), 1, colors.black)
        ]))
        story.append(table)

    def _build_portfolio_recommendation(self, story: List[Any], analysis: Dict[str, Any]):
        """Builds the top recommendations table."""
        story.append(Paragraph("Top Portfolio Recommendations", self.styles['SectionHeader']))
    ...
    def _build_risk_assessment(self, story: List[Any], analysis: Dict[str, Any]):
        ...
        story.append(Paragraph(f"<b>Overall Risk:</b> {analysis['risk_assessment']['overall_risk']}", self.styles['NormalLeft']))
        story.append(Paragraph(f"<b>Recommendation:</b> {analysis['risk_assessment']['recommendation']}", self.styles['NormalLeft']))
        story.append(Spacer(1, 12))
    ...
    def _build_next_steps(self, story: List[Any]):
        ...
        steps = [
            "<b>1. Validate Spend:</b> Review the machine-specific spend distribution to confirm stability.",
            "<b>2. Start Conservatively:</b> Implement the top 3-5 recommendations for high-spend, low-risk machine types.",
            "<b>3. Monitor Utilization:</b> Set up monitoring for CUD utilization before expanding commitments.",
            "<b>4. Consider Flex CUDs:</b> Use Flex CUDs for workloads with less predictable usage patterns.",
            "<b>5. Quarterly Review:</b> Establish a quarterly cadence to review this report and adjust your CUD portfolio."
        ]
        for step in steps:
