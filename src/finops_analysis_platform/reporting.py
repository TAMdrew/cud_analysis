...
def create_dashboard(analysis: Dict[str, Any], config_manager: ConfigManager):
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=(
            'Monthly Savings by Strategy', 'Machine Type Spend Distribution',
    ...
    strategies = [
        '1-Yr Resource', '3-Yr Resource', '1-Yr Flex', '3-Yr Flex', 'Optimal Mix'
    ]
    savings = [
    ...
    fig.add_trace(
        go.Bar(
            x=strategies, y=savings, text=[f'${s:,.0f}' for s in savings],
            textposition='auto',
            marker_color=[
    ...
    top_machines = sorted(
        analysis['machine_spend_distribution'].items(),
        key=lambda x: x[1],
        reverse=True
    )[:8]
    fig.add_trace(
    ...
    fig.add_trace(
        go.Bar(
            x=['Low', 'Medium', 'High'], y=risk_values,
...
class PDFReportGenerator:
    ...
    def __init__(self, config_manager: ConfigManager):
        ...
        self.theme = self.config_manager.get(
            'reporting.theme_colors',
            {
                ...
                'accent': '#F59E0B', 'background': '#F3F4F6',
                'text': '#111827'
            }
        )
    ...
    def _setup_custom_styles(self):
        ...
        self.styles.add(ParagraphStyle(
            name='CustomTitle', fontSize=24,
            textColor=HexColor(self.theme['primary']), spaceAfter=20,
            alignment=TA_CENTER, fontName='Helvetica-Bold'
        ))
        self.styles.add(ParagraphStyle(
            name='SectionHeader', fontSize=16,
            textColor=HexColor(self.theme['primary']), spaceAfter=12,
    ...
    def _build_title_page(self, story: List[Any]):
        ...
        story.append(Paragraph(
            "Cloud FinOps CUD Analysis Report", self.styles['CustomTitle']
        ))
    ...
    def _build_portfolio_recommendation(
        ...
        story.append(Paragraph(
            "Top Portfolio Recommendations", self.styles['SectionHeader']
        ))
    ...
    def _build_next_steps(self, story: List[Any]):
        ...
        story.append(Paragraph(
            "Actionable Next Steps", self.styles['SectionHeader']
        ))
        steps = [
